...
static char nextc()
{ // if (30+40) expression will point to the whole expression inside the buffer
...
const char *read_number_str() // reads number and writes it onto a buffer
{
    const char *num = NULL;
...
unsigned long long read_number() // reads token and converts
{
    const char *s = read_number_str();
    return atoll(s); // converts to a long long
}
...
struct token *token_make_number_for_value(unsigned long number) // Creates token and writes the value in it
{
    int number_type = lexer_number_type(peekc()); // for number like 7612L or 52f
...
static struct token *token_make_string(char start_delim, char end_delim)
{
    ...
    assert(nextc() == start_delim); // to get rid of the " before string
    char c = nextc();
    for (; c != end_delim && c != EOF; c = nextc())
    { // just end condition and action every step
        if (c == '\\')
        {
            // We need to handle an escape character like '\n'
            continue;
        }
        buffer_write(buf, c);
    }
    buffer_write(buf, 0x00); // attaching null terminator at the end
    return token_create(&(struct token){.type = TOKEN_TYPE_STRING, .sval = buffer_ptr(buf)});
}

static bool op_treated_as_one(char op)
{ // tells us if op is only used once not like &&
...
{ // checks if operator is valid and returns true
    return S_EQ(op, "+") ||
           S_EQ(op, "-") ||
           S_EQ(op, "*") ||
           S_EQ(op, "/") ||
...
{
    lex_process->current_expression_count--; // because last elemement is going to be an expression like )
    if (lex_process->current_expression_count < 0)
    {
        // if closing bracket after none opening ones
        compiler_error(lex_process->compiler, "You closed an expression that never opened\n");
    }
}
...
struct token *token_make_one_line_comment()
{
    struct buffer *buffer = buffer_create();
    char c = 0;
    LEX_GETC_IF(buffer, c, c != '\r' && c != EOF);
...
{ // chooses to which comment function you'll be redirected
    char c = peekc();
    if (c == '/')
    {
        nextc();
...
struct token *token_make_special_number_hexadecimal()
{
    ...
    // convert str in understandable number for computer
...
struct token *token_make_special_number() // hexa and binary
{

    struct token *token = NULL;
    struct token *last_token = lexer_last_token();
    if (!last_token || !(last_token->type == TOKEN_TYPE_NUMBER && last_token->llnum == 0))
    { // Checks if there is a 0 before an x or b
        return token_make_identifier_or_keyword();
    }

    lexer_pop_token(); // popping of the 0
...
struct token *read_next_token()
{
    ...
    case 'x': // the 0 from 0x7526 is getting deleted once getting in here
        token = token_make_special_number();
        break;
...
char lexer_string_buffer_push_char(struct lex_process *process, char c) //Gives warning if void is used
{
    struct buffer *buf = lex_process_private(process);
    buffer_write(buf, c);
}
