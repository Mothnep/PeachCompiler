...
void parser_reorder_expression(struct node **node_out)
{
    ...
    if (node->exp.left->type != NODE_TYPE_EXPRESSION && node->exp.right && node->exp.right->type == NODE_TYPE_EXPRESSION)
    {
        ...
        { /* 50 in example above is not an expression,
...
void parse_datatype_type(struct datatype* dtype){
    struct token* datatype_token = NULL;  //if struct abc   then secondary token will still be NULL
    ...
    {
        if (token_peek_next()->type == TOKEN_TYPE_IDENTIFIER) // Looking for name of variable
        {
            datatype_token = token_next(); //leave the struct take the abc name
        }
        else
        {
            // This structure has no name, so we need to handle it. // like struct{}abc
